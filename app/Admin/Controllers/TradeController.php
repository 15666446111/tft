...
class TradeController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('trade_no',__('流水号'))->help('交易订单的流水号,腾付通支付的订单号');

        $grid->column('agt_merchant_id', __('渠道商'))->help('交易订单所归属的渠道商机构号');

        // $grid->column('rrn', __('参考号'))->help('交易订单的参考号,腾付通支付的参考号');

        $grid->column('users.nickname', __('所属代理'))->help('交易订单所归属的代理昵称');

        $grid->column('sn', __('序列号'))->help('交易订单所归属的终端机具SN');
        ...
        $grid->column('tran_code', __('交易类型'))->using([
            '0' =>  '银行卡', '1' => '银行卡闪付', '2' => '微信支付', '3' => '支付宝', '4' => '银联二维码'
        ])->help('当前交易订单的交易类型');
        ...
        $grid->column('card_type', __('交易卡'))->using([
            '0' => '借记卡', '1' => '贷记卡'
        ])->label([
            '0' =>  'warning', '1' => 'success'
        ])->help('当前交易订单的交易卡类型');

        $grid->column('is_send', __('分润'))->bool()->help('当前交易订单是否发放分润');
