...
{	
    ...
    {
        $Arr =  $this->getMyTeam();
        return \App\Merchant::whereBetween('created_at', [ $this->StartTime,  $this->EndTime])->whereIn('user_id', $Arr)->where('verfity_state', 1)->count();     
    }
    ...
    public function getTradeSum($rule = 'team')
    {
        ...
        return \App\Trade::whereBetween('trade_time', [ $this->StartTime,  $this->EndTime])
                ->whereHasIn('users', function($q) use ($Arr){
                    $q->whereIn('id', $Arr);
                })->where('is_repeat', 0)->where('is_invalid', 0)->sum('amount');
    }
