...
class RepayCjController extends Controller
{
    ...
    public function __construct( Withdraw $Withdraw )
    {
        ...
        if($adminSetting->payment_type=="1"){
            ...
            $this->desc    =  "工作日:代付扣单笔提现费:".number_format( $this->chbPrice /100 ,2, '.', ',')."元!";
        }else{

            if($this->isHoliday){
                $this->cjPrice =    $this->withdraw->real_money;
                $this->desc    =    "节假日:".$holiday->title."/代付不扣单笔提现费!";
            }else{
                $this->cjPrice =  $this->withdraw->real_money + $this->price;
                $this->desc    =  "工作日:代付扣单笔提现费:".number_format( $this->price /100 ,2, '.', ',')."元!";
            }
        }
    }
    ...
    public function chbPay($bankNo, $cardNo)
    {
        ...
        if($tokenResult['code'] != "200" or !isset($tokenResult['token']) ){
            $this->withdraw->api_return_data = json_encode(array('code' => '10100', 'message' => "代付出错: 未获取到token!" ));
            $this->withdraw->save();
            throw new \Exception(" 代付出错: 未获取到token!");
        }
        ...
        if(!is_array($content)){
            $this->withdraw->api_return_data = json_encode(array('code' => '10090', 'message' => "代付出错: 返回非数组类型!" ));
            $this->withdraw->save();
            throw new \Exception(" 代付出错: 返回非数组类型!");
        } 

        if(!$content['code']){
            $this->withdraw->api_return_data = json_encode(array('code' => '10090', 'message' => "代付出错: 返回非标识码!" ));
            $this->withdraw->save();
            throw new \Exception(" 代付出错: 返回非标识码!");
        } 
        //echo json_encode($postData).$this->chanKey."<br/>";
        if($content['code'] != "200" ){
            $this->withdraw->api_return_data = json_encode(array('code' => '10090', 'message' => "代付出错: ".$content['msg'] ));
            $this->withdraw->save();
            throw new \Exception(" 代付出错:".$content['msg']);
        }
